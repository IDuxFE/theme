...
import { type ComputedRef, computed, effectScope, inject, onScopeDispose } from 'vue-demi'

import { useSharedProviderContext } from './composables/useThemeProviderContext'
...
export interface ScopedUseThemeTokenContext<
...
> {
  ...
  registerToken: {
    ...
    (registry: ThemeRegistry<Ext, K>): string
  }
}
...
export interface UseThemeToken<Ext extends ExtraTokens = ExtraTokens> {
  (): UseThemeTokenContext<undefined, Ext>
  <K extends UseThemeTokenScope | keyof Ext>(key: K): UseThemeTokenContext<K, Ext>
}
...
export function useThemeToken<
  ...
  K extends UseThemeTokenScope | keyof Ext | undefined,
>(key: K): UseThemeTokenContext<K, Ext>
export function useThemeToken<
  Ext extends ExtraTokens,
  K extends UseThemeTokenScope | keyof Ext | undefined,
>(key?: K): UseThemeTokenContext<K, Ext> {
  ...
  if (!themeProviderContext) {
    themeProviderContext = useSharedProviderContext()
  }
  ...
  const context = getSharedContext(key, useThemeTokenContextMap, () => {
    ...
    const registerToken = (
      getTokens: TokenGetter<NotNullKey, Ext> | ThemeRegistry<Ext, NotNullKey>,
      transforms?: TokenTransforms<NotNullKey, Ext>,
      prefixCls?: string,
    ) => {
      const registry = isFunction(getTokens)
        ? {
            getter: getTokens,
            transforms: transforms as TokenTransforms<ThemeKeys>,
            prefix: prefixCls,
          }
        : getTokens
      return _registerToken(key, registry)
    }
...
function getSharedContext<K extends ThemeKeys | keyof Ext, Ext extends ExtraTokens = ExtraTokens>(
  key: K,
  map: Map<ThemeKeys | keyof Ext, UseThemeProviderStates<Ext> | undefined>,
...
export function extendUseThemeToken<Ext extends ExtraTokens>(): UseThemeToken<Ext> {
  return useThemeToken as unknown as UseThemeToken<Ext>
}